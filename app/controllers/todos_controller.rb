 ...
 #global string to use as the suffix for the element id for todo's <UL>
  ...
  def toggle_complete
    ...
    if (request.xhr?)
		@element_html = render_to_string :partial => 'todos/todo',
										 :locals => {:todo => @todo, :editable => true}
		respond_to do |format|
			format.js { render :template => "todos/todo.js", :layout => false }
		end
    else
		respond_to do |format|
		...
		if @todo.save
		  if (request.xhr?)
			@element_html = render_to_string :partial => 'todos/todo_li',
											 :locals => { :todo => @todo, :editable => true }
			format.js { render :template => "todos/create.js", :layout => false }   #using rjs
		  else
		  ...
		  format.html { render :action => "index", :project_id => params[:project_id] }
		end
	end
  end
  ...
  def sort
	...
	if params.has_key?("todo")
		...
		# update the todos parent id, position
		#@todos.each do |todo|
			# retrieve the new positions for current todo
		#	new_todo_details = todo_params
		#end

		#params["todo"].keys.sort.each do |position|
		#	new_todo_details = params["todo"][position]
		#end
